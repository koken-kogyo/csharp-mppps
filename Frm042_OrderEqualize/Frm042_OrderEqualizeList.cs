    ...
    public partial class Frm042_OrderEqualize : Form
    {
        ...
        private void DeleteCuttingProdInfofromList(int rowCnt, ref int[] delRowCnt)
        {
            ...
            else if (dgvRow.Cells[(int)Common.Frm042DgvIdx.TableName].Value.ToString().Trim() == Common.TABLE_ID_KD8440)
            {
                ...
                // テーブルからデータを削除
                ret = cmn.Dba.DeleteCuttingProdScheduleInfo();
            ...
            if (ret > 0)
            {
                ...
                // DataGridView から行を削除
                Dgv_MpSimOdrTbl.Rows.RemoveAt(rowCnt);
            }
            else if (ret == 0)
            {
                // 該当データなし
        ...
        private void SetCuttingProdPlanPKeyfromList(int rowCnt)
        {
            ...
            if (cmn.ButtonText == Common.FRM_BUTTON_TEXT_DEL)
            {
                // 削除のときキー値の取得は現状 DataGridView から行う
                dgvRow = Dgv_MpCurOdrTbl.Rows[rowCnt];
            }
            else
            {
                // 削除以外のときキー値の取得は変更後 DataGridView から行う
                dgvRow = Dgv_MpSimOdrTbl.Rows[rowCnt];
            }
                ...
                for (int rowCnt = 0; rowCnt < dgvRowCnt; rowCnt++)
                {
                    ...
                    if (dgvRow.Cells[(int)Common.Frm042DgvIdx.TableName].Value.ToString().Trim() == Common.TABLE_ID_KD8430)
                    {
                        ...
                        if (dataSet.Tables[0].Rows.Count > 0) // データあり
                        {
                            ...
                            // データを更新
                            if (cmn.Dba.MergeCuttingProdPlanInfo() > 0)
                            {
                                // 更新成功
                    ...
                    else
                    {
                        ...
                        if (dataSet.Tables[0].Rows.Count > 0) // データあり
                        {
                            ...
                            // データを更新
                            if (cmn.Dba.MergeCuttingProdScheduleInfo() > 0)
                            {
                                // 更新成功
                                updRowCnt[Common.IDX_NORMAL_END]++;
                            }
        ...
        private void DeleteCuttingProdInfoList()
        {
            ...
            if (dialogResult == DialogResult.OK) // [OK] クリック時
            {
                ...
                {
                    // 切削生産計画ファイル削除
                    DeleteCuttingProdInfofromList(src[i].Index, ref delRowCnt);
